...
class Events extends Component {
  ...
  category = this.props.for.id;
  componentDidMount() {
    api.getEventsForCategory(this.props.for, {
      onSuccess: (value) =>{
        this.setState({events: value, cardsLoading: false})
  ...
  returnCards = () => {
    ...
    for(let j = 0; j<this.state.events.length; j++){
        ...
        <View  key={j} style={{justifyContent: 'center'}}>
          <EventCard onRegister={this.props.onRegister} event={this.state.events[j]} />
        </View>
      )
    }
    return cards;
...
export default class EventChoosenType extends Component{
        ...
        organiserViews.push(
          <View key={i} style={{flex: 1}}>
            <View style = {{backgroundColor: colors.teal, flex:1}}>
               <Text style={styles.organiserHadding}>{sub_categories[i].name}</Text>
            </View>
            <View style={{height: 300, width: Dimensions.get('window').width, justifyContent: 'center', alignItems: 'center'}}>
              <Events for={sub_categories[i]} onRegister={this.openDetails.bind(this, 'true')} />
            </View>
          </View>
        )
      }
    ...
    if(this.state.carousel == 'true'){
      return (
        ...
        <View style={styles.navBar}>
          <TouchableNativeFeedback onPress={this.handleClick.bind(this, 'true')} background={TouchableNativeFeedback.Ripple('#ffffff', true)}>
