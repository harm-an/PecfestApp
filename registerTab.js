...
const colors = { selected: '#ff5a5f', normal: '#484848' , teal: '#008489', StatusBarTeal: '#066f73', separator: '#ebebeb', dull: '#ebebeb'};
...
export default class RegisterTab extends Component {
    ...
    for(let j = 0; j<this.state.teamSize; j++){
      if(j == 0){
        inputs.push(
          <TextInput key={j}
                  autoGrow={false}
                  style={{ width: 160, borderWidth: 0, borderColor: 'white', textAlign: 'center',fontFamily: 'Montserrat-Regular', fontSize: 22, height: 48, color: colors.teal}}
  ...
  render() {
    return (
        <ScrollView onStartShouldSetResponderCapture={(evt)=>true} style={styles.scrollContainer} contentContainerStyle={{alignItems: 'center'}}>
          <Text style={{color: colors.teal, fontFamily: 'Montserrat-Medium', fontSize: 22, marginTop: 16}}>Register</Text>
          <Text style={{color: colors.teal, fontFamily: 'Montserrat-Regular', fontSize: 12, marginBottom: 16}}>Team Size: {this.props.event.minSize} - {this.props.event.maxSize}</Text>
          <Text style={{color: colors.teal, fontFamily: 'Montserrat-Medium', fontSize: 12, marginTop: 16}}>Team Member(s)</Text>
          {this.returnInputs()}
          <TouchableWithoutFeedback background={TouchableNativeFeedback.Ripple(colors.teal, true)} onPress={this.handleAddMember}>
            <Image source={require('./icons/ic_person_add.png')} style={{tintColor: colors.teal}}/>
          </TouchableWithoutFeedback>
          { this.props.event.maxSize > 1 ?
            <View style={{alignItems: 'center'}}>
              <Text style={{color: colors.teal, fontFamily: 'Montserrat-Medium', fontSize: 12, marginTop: 16}}>Leader ID ({this.state.leaderId})</Text>
              <Picker style={{ height: 50, width: Dimensions.get('window').width, marginLeft: 16}} selectedValue={this.state.leaderId} onValueChange={this.handleLeaderChange} mode="dropdown">
                {
          ...
          <TouchableWithoutFeedback  background={TouchableNativeFeedback.Ripple(colors.teal, true)} onPress={this.handleSubmit}>
          ...
          <Text style={{color: colors.teal, fontFamily: 'Montserrat-Regular', fontSize: 12, marginBottom: 16, color: colors.selected}}>{this.state.statusMessage || ''}</Text>
        </ScrollView>
      )
  }
}
